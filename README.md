...
Once we have this setup the actor system can be started with the same name, this will start cluster communication and there will be a leader node and other nodes joining. All these are managed by AKKA framework, we just have to configure and start the actor system on all nodes
...
It can also be a fleet structure where there can be millions of actors that will serve for an entity, all in-memory procedssing, for example in an inventory system with thousands of stores with hundreds of items, there can be an actor for each store-item combination waiting to process inventory transactions instantanously
